...
RSpec.describe Authorizer::AccountStatement::Violations::
  ...
  describe '#violation?' do
    ...
    let(:event) { { time: '2019-02-13T10:00:00.000Z', merchant: merchant, amount: amount } }
    ...
    it 'allows transactions to same merchant with different amount in a 2 minutes interval' do
      expect(
        subject.violation?(
          event: { time: '2019-02-13T10:00:00.000Z', merchant: merchant, amount: amount + 1 },
      ...
      expect(
        subject.violation?(
          event: { time: '2019-02-13T10:02:00.000Z', merchant: merchant, amount: amount + 1 },
          statements_history: [transaction]
        )
      ).to be false
    end
    ...
    it 'allows transactions to same merchant with same amount in an interval higher than 2 minutes' do
      expect(
        subject.violation?(
          event: { time: '2019-02-13T10:02:01.000Z', merchant: merchant, amount: amount },
          statements_history: [transaction]
        )
      ).to be false
    end
    ...
    it 'violates when trying to do similar transactions in a 2 minutes interval' do
      expect(
        subject.violation?(
          event: { time: '2019-02-13T10:00:00.000Z', merchant: merchant, amount: amount },
          statements_history: [transaction]
        )
      ).to be true
      ...
      expect(
        subject.violation?(
          event: { time: '2019-02-13T10:02:00.000Z', merchant: merchant, amount: amount },
          statements_history: [transaction]
        )
      ).to be true
    end
